...
describe Bosh::AzureCloud::AzureClient2 do
  ...
  describe "#create_network_interface" do
    ...
    context "when token is valid, the ip address is in the backend pool, create operation is accepted and completed" do
      it "should create a network interface without error" do
        ...
        expect {
          azure_client2.delete_backend_address_of_application_gateway(ag_name, backend_address)
        }.not_to raise_error
      end
    end

    context "when the ip address is not in the backend pool," do
      it "should not update the application gateway" do
        ...
        stub_request(:get, application_gateway_uri).to_return(
          :status => 200,
          :body => ag_not_included_target_ip.to_json,
          :headers => {})
